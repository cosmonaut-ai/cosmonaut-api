...
def handler(event: SQSEvent, context: Any) -> HandlerReturn:
  ...
  for record in event.records:
    try:
      # 1. Parse Body
      payload = SQSPayload.model_validate_json(record.body)
...
async def _process_task(payload: SQSPayload):
  ...
  if task_type == "analyze_node":
    analyze_node_payload = AnalyzeNodePayload.model_validate(payload)
    world_id = analyze_node_payload.world_id
    node_id = analyze_node_payload.node_id
    # FAST LANE TASK (~3s)
  ...
  elif task_type == "generate_world":
    # SLOW LANE TASK (~60s)
    # Generates Lore -> Narrator -> Start Node
    generate_world_payload = GenerateWorldPayload.model_validate(payload)
    world_id = generate_world_payload.world_id
    if not world_id:
  ...
  elif task_type == "generate_world_image":
    generate_world_image_payload = GenerateWorldImagePayload.model_validate(payload)
    world_id = generate_world_image_payload.world_id
    # SLOW LANE TASK (~15s)
