...
async def create_session(response: Response, current_user: User = Depends(get_current_user)):
  ...
  ph_capture("user_session_created", distinct_id=current_user.id, properties={"tier": current_user.tier})
...
async def create_checkout(
  ...
  ph_capture("checkout_initiated", distinct_id=current_user.id, properties={"tier": payload.tier})
...
async def delete_user_account(current_user: User = Depends(get_current_user)) -> dict[str, str]:
  ...
  try:
    ...
    ph_capture("account_deleted", distinct_id=current_user.id)
...
async def update_newsletter(
  ...
  ph_capture("newsletter_preference_updated", distinct_id=current_user.id, properties={"opted_in": payload.opted_in})
